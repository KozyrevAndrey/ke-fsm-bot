...
async def message2_stuff(message: types.Message, state: FSMContext):
    ...
    await message.answer("Напишите адрес и номер ПВЗ, на который нужна замена?\n\nПример: Улица Профсоюзная 1, КЗН-1")
...
async def message3_stuff(message: types.Message, state: FSMContext):
    ...
    await message.answer("Со сколько до скольки вам нужна замена?\n\nПример: 15:00-18:00 или 15-18")
...
async def send_info(message: types.Message, state: FSMContext):
    ...
    await message.bot.send_message(CHANNEL_NAME, f"Требуется замена на дату: <b>{data['message_group1']}</b>,\n\n"
...
def register_handlers_change(dp: Dispatcher):
    ...
    dp.register_message_handler(message1_stuff, Text(equals="Нужна замена", ignore_case=True), state="*")
