...
from app.Message_fsm import MessageState, CHANNEL_NAME
...
async def message_help2(message: types.Message, state: FSMContext):
    ...
    await message.answer("Напишите адрес и номер ПВЗ, на который нужен доп.сотрудник?\n\nПример: Улица Профсоюзная 1, КЗН-1")
...
async def message_help3(message: types.Message, state: FSMContext):
    ...
    await message.answer("Со скольки до скольки вам нужен доп.сотрудник?\n\nПример: 15:00-18:00 или 15-18")
    await MessageState.message6.set()

async def send_help(message: types.Message, state: FSMContext):
    await state.update_data(group3=message.text)
    change_data = await state.get_data()
    await message.bot.send_message(CHANNEL_NAME, f"Требуется дополнительный сотрудник на дату: <b>{change_data['group1']}</b>,\n\n"
...
def register_handlers_stuff(dp: Dispatcher):
    ...
    dp.register_message_handler(message_help1, Text(equals="Нужен доп.сотрудник", ignore_case=True), state="*")
